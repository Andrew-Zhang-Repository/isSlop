...
export function checkMetadata(arrayBuffer) {
  if (typeof ExifReader === "undefined") return { detected: false, reason: "exifreader-unavailable" };
  try {
    const tags = ExifReader.load(arrayBuffer);           // ArrayBuffer, not Uint8Array
    const fields = [tags.Software?.description, tags.CreatorTool?.description,
                    tags.UserComment?.description, tags.Parameters?.description,
                    tags.History?.description, tags.Artist?.description];
    ...
    return { detected: false, reason: `exif-error:${e?.message ?? e}` };
  }
}